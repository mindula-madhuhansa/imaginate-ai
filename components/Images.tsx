...
function Images() {
  ...
  return (
    <div>
      <button
        className="fixed bottom-10 right-10 bg-blue-400/90 text-white px-5 py-3 rounded-md hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-400 font-bold z-20"
      ...
      {isLoading && (
        <p className="animate-pulse text-center pb-7 font-extralight">
          Loading <span className="text-blue-600">AI</span> Generated Images...
        </p>
      )}
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 px-0 md:px-10">
        {images?.imageUrls?.map((image: ImageType, i: number) => (
          ...
          >
            <div className="absolute flex justify-center items-center h-full w-full bg-white opacity-0 hover:opacity-80 transition-opacity duration-200 z-10">
